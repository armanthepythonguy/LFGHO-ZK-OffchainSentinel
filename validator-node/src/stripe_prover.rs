...
const SERVER_DOMAIN: &str = "api.stripe.com";
...
async fn main() {
    ...
    let payment_id = env::var("PAYMENT_ID").unwrap();
    ...
    .uri(format!(
        //"https://{SERVER_DOMAIN}/v1/checkout/sessions/{payment_id}"
        "https://{SERVER_DOMAIN}/v1/balance"
    ))
    ...
    let mut file = tokio::fs::File::create("stripe_notarized.json")
        .await
        .unwrap();
    file.write_all(
    ...
    let mut file = tokio::fs::File::create("stripe_proof.json")
        .await
        .unwrap();
...
async fn setup_notary_connection() -> (tokio_rustls::client::TlsStream<TcpStream>, String) {
    ...
    let pem_file =
        str::from_utf8(include_bytes!("../certificates/rootCA.crt")).unwrap();
